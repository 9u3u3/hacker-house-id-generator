...
export async function POST(req: Request) {
  ...
  /* the roster is untrusted text that ends up on a rendered page — clamp both
     the count and each field rather than trusting whatever was posted */
  const members: CrewMemberRecord[] | undefined = Array.isArray(meta.members)
    ? (meta.members as unknown[]).slice(0, 3).map((raw) => {
        const m = (raw ?? {}) as Partial<CrewMemberRecord>;
        return {
  ...
  const record: PassRecord = {
    ...(meta.team ? { team: String(meta.team).slice(0, 60) } : {}),
    ...
    tier: String(meta.tier ?? "").slice(0, 12),
